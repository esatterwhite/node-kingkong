 ...
 * @param {module:kingkong~Api[]} [options.apis] an array of api configurations to be created or updated when the instance is created
...
King = new Class({
    ...
    ,plugins: co.wrap(function*( id, data ){
        var url, plugins_list,plugins, to_do, resp;
        ...
        for( let idx=0,len=plugins.length; idx<len; idx++){
            ...
            if( to_do.hasOwnProperty( plugin.name ) ){
                ...
                debug('deleteing stale plugin', plugin.name)
                resp = yield request({
                    method:'DELETE'
